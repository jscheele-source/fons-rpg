...
# Milestone 1 dialogue pass: same quests, choices, and consequences as the
# foundation NPCs, but with shorter and more conversational delivery.
...
func _cael_dialogue() -> Dictionary:
    ...
    if current_page == "task":
        return {
            "speaker": display_name,
            "text": "There's an old annex east of the court. A resonator core was left inside when the work stopped. Bring it back. If anything in there starts sounding familiar, leave first and be curious later.",
...
func _sera_dialogue() -> Dictionary:
    ...
    if q["state"] == "active" and GameState.has_item("resonator_core"):
        return {
            "speaker": display_name,
            "text": "That's the resonator, isn't it? Cael will want it inside. I'd like to measure it before it disappears into storage.",
            "choices": [
    ...
    if q["state"] == "completed" and q["resolution"] == "flamen":
        return {"speaker": display_name, "text": "You gave it to Cael. Fair enough. I hope somebody measures it before they lock it away.", "choices": [{"text": "Goodbye.", "action": "close"}]}
    return {
        "speaker": display_name,
...
func _novice_dialogue() -> Dictionary:
    if current_page == "novice_lucretia":
        return {"speaker": display_name, "text": "People lower their voices when her name comes up. That's about all I know for certain.", "choices": [{"text": "Back.", "action": "back"}]}
    ...
    if current_page == "novice_war":
        return {"speaker": display_name, "text": "I don't know. There are more supply crates than there were last month. Nobody tells novices why.", "choices": [{"text": "Back.", "action": "back"}]}
    ...
    var choices: Array = [
        ...
        {"text": "What's with all the new crates?", "action": "novice_war"},
    ]
