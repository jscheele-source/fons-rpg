...
func _build_entry_hall(root: Node3D) -> void:
    var center := Vector3(0, 0, 13)
    _room_floor(root, "EntryFloor", center + Vector3(0, 0.06, 0), Vector3(10.4, 0.12, 10.4))
    _ceiling(root, "EntryCeiling", center + Vector3(0, 4.85, 0), Vector3(10.4, 0.28, 10.4))
    _wall(root, "EntryWest", center + Vector3(-5.1, 2.4, 0), Vector3(0.55, 4.8, 10.7))
    _wall(root, "EntryEast", center + Vector3(5.1, 2.4, 0), Vector3(0.55, 4.8, 10.7))
    _wall_x_with_door(root, "EntrySouth", center + Vector3(0, 0, 5.1), 10.7, 3.2, 4.8)
    _wall_x_with_door(root, "EntryNorth", center + Vector3(0, 0, -5.1), 10.7, 4.8, 4.8)
    _door_trim_x(root, center + Vector3(0, 0, -5.42), 4.8, 3.35)
    _brazier(root, center + Vector3(-3.3, 0, -2.5))
    _brazier(root, center + Vector3(3.3, 0, -2.5))
    _warm_light(root, center + Vector3(0, 3.6, 0), 1.9, 9.0)

    var exit_door = DOOR.new()
    exit_door.display_name = "Outer Courtyard"
    exit_door.position = center + Vector3(0, 0, 4.82)
    ...
    _corridor_z(root, Vector3(0, 0, 5.35), 4.8, 5.8, true)

func _build_junction(root: Node3D) -> void:
    var center := Vector3(0, 0, 0)
    _room_floor(root, "JunctionFloor", center + Vector3(0, 0.06, 0), Vector3(8.6, 0.12, 6.6))
    _ceiling(root, "JunctionCeiling", center + Vector3(0, 4.7, 0), Vector3(8.6, 0.28, 6.6))
    _wall_x_with_door(root, "JunctionSouth", center + Vector3(0, 0, 3.2), 8.8, 4.8, 4.65)
    _wall_x_with_door(root, "JunctionNorth", center + Vector3(0, 0, -3.2), 8.8, 4.6, 4.65)
    _wall_z_with_door(root, "JunctionWest", center + Vector3(-4.2, 0, 0), 6.8, 4.2, 4.65)
    _wall_z_with_door(root, "JunctionEast", center + Vector3(4.2, 0, 0), 6.8, 4.2, 4.65)
    _warm_light(root, center + Vector3(0, 3.5, 0), 1.25, 8.0)
    _corridor_x(root, Vector3(-6.1, 0, 0), 4.2, 4.2, true)
    _corridor_x(root, Vector3(6.1, 0, 0), 4.2, 4.2, true)
    _corridor_z(root, Vector3(0, 0, -6.05), 4.6, 6.1, true)

func _build_library(root: Node3D) -> void:
    var center := Vector3(-13.1, 0, 0)
    _room_floor(root, "LibraryFloor", center + Vector3(0, 0.06, 0), Vector3(10.6, 0.12, 12.6))
    _ceiling(root, "LibraryCeiling", center + Vector3(0, 4.8, 0), Vector3(10.6, 0.28, 12.6))
    _wall(root, "LibraryWest", center + Vector3(-5.2, 2.4, 0), Vector3(0.55, 4.8, 12.9))
    _wall(root, "LibraryNorth", center + Vector3(0, 2.4, -6.2), Vector3(10.9, 4.8, 0.55))
    _wall(root, "LibrarySouth", center + Vector3(0, 2.4, 6.2), Vector3(10.9, 4.8, 0.55))
    _wall_z_with_door(root, "LibraryEast", center + Vector3(5.2, 0, 0), 12.9, 4.2, 4.8)
    _corner_trim(root, center + Vector3(-5.2, 0, -6.2), 4.8)
    _corner_trim(root, center + Vector3(-5.2, 0, 6.2), 4.8)

    # Shelf fronts face into the room. Default shelf front is +Z.
    _shelf(root, center + Vector3(-3.6, 0, -3.7), 90.0)
    _shelf(root, center + Vector3(-3.6, 0, 0.0), 90.0)
    _shelf(root, center + Vector3(-3.6, 0, 3.7), 90.0)
    _shelf(root, center + Vector3(1.2, 0, -4.9), 0.0)
    _shelf(root, center + Vector3(1.2, 0, 4.9), 180.0)
...
func _build_dormitory(root: Node3D) -> void:
    var center := Vector3(13.1, 0, 0)
    _room_floor(root, "DormFloor", center + Vector3(0, 0.06, 0), Vector3(10.6, 0.12, 16.6))
    _ceiling(root, "DormCeiling", center + Vector3(0, 4.55, 0), Vector3(10.6, 0.28, 16.6))
    _wall(root, "DormEast", center + Vector3(5.2, 2.3, 0), Vector3(0.55, 4.6, 16.9))
    _wall(root, "DormNorth", center + Vector3(0, 2.3, -8.2), Vector3(10.9, 4.6, 0.55))
    _wall(root, "DormSouth", center + Vector3(0, 2.3, 8.2), Vector3(10.9, 4.6, 0.55))
    _wall_z_with_door(root, "DormWest", center + Vector3(-5.2, 0, 0), 16.9, 4.2, 4.6)
    _corner_trim(root, center + Vector3(5.2, 0, -8.2), 4.6)
    _corner_trim(root, center + Vector3(5.2, 0, 8.2), 4.6)

    _partition_x_with_opening(root, center + Vector3(0, 0, -2.7), 10.0, 2.4)
    _partition_x_with_opening(root, center + Vector3(0, 0, 2.7), 10.0, 2.4)
    ...
    _wall_lamp(root, center + Vector3(4.8, 2.6, -5.4))
    _wall_lamp(root, center + Vector3(4.8, 2.6, 0.0))
    _wall_lamp(root, center + Vector3(4.8, 2.6, 5.4))

func _build_meditation_court(root: Node3D) -> void:
    var center := Vector3(0, 0, -15.0)
    _room_floor(root, "MeditationStone", center + Vector3(0, 0.05, 0), Vector3(14.6, 0.10, 12.6), STONE_MID)
    _box(root, "MeditationSoil", center + Vector3(0, 0.09, 0), Vector3(7.2, 0.08, 6.6), Color(0.30, 0.20, 0.20), false)
    _wall(root, "MeditationWest", center + Vector3(-7.2, 2.5, 0), Vector3(0.55, 5.0, 12.9))
    _wall(root, "MeditationEast", center + Vector3(7.2, 2.5, 0), Vector3(0.55, 5.0, 12.9))
    _wall_x_with_door(root, "MeditationSouth", center + Vector3(0, 0, 6.2), 14.9, 4.8, 5.0)
    _wall_x_with_door(root, "MeditationNorth", center + Vector3(0, 0, -6.2), 14.9, 4.8, 5.0)
    _corner_trim(root, center + Vector3(-7.2, 0, -6.2), 5.0)
    _corner_trim(root, center + Vector3(7.2, 0, -6.2), 5.0)
    _corner_trim(root, center + Vector3(-7.2, 0, 6.2), 5.0)
    _corner_trim(root, center + Vector3(7.2, 0, 6.2), 5.0)
    _door_trim_x(root, center + Vector3(0, 0, 5.88), 4.8, 3.35)
    _door_trim_x(root, center + Vector3(0, 0, -5.88), 4.8, 3.35)
    ...
    # The altar belongs to the west wall, leaving the north passage entirely clear.
    _box(root, "MeditationAltar", center + Vector3(-6.65, 1.05, -0.4), Vector3(0.60, 1.8, 4.4), STONE_PALE)
    for z in [-1.65, -0.85, -0.05, 0.75, 1.55]:
        _candle(root, center + Vector3(-6.30, 2.05, z - 0.4))
    _chimes_z(root, center + Vector3(-6.25, 3.15, -2.0))
    _chimes_z(root, center + Vector3(-6.25, 3.15, 1.2))
    _warm_light(root, center + Vector3(-5.5, 2.5, -0.4), 1.35, 7.0)
...
func _wall_x_with_door(root: Node3D, prefix: String, base: Vector3, total_width: float, opening_width: float, height: float) -> void:
    var segment: float = (total_width - opening_width) * 0.5
    var offset: float = opening_width * 0.5 + segment * 0.5
    _wall(root, prefix + "L", base + Vector3(-offset, height * 0.5, 0), Vector3(segment + 0.12, height, 0.55))
    _wall(root, prefix + "R", base + Vector3(offset, height * 0.5, 0), Vector3(segment + 0.12, height, 0.55))
    var door_height := 3.35
    var lintel_height: float = height - door_height
    _wall(root, prefix + "Lintel", base + Vector3(0, door_height + lintel_height * 0.5, 0), Vector3(opening_width + 0.16, lintel_height, 0.55))

func _wall_z_with_door(root: Node3D, prefix: String, base: Vector3, total_length: float, opening_width: float, height: float) -> void:
    var segment: float = (total_length - opening_width) * 0.5
    var offset: float = opening_width * 0.5 + segment * 0.5
    _wall(root, prefix + "N", base + Vector3(0, height * 0.5, -offset), Vector3(0.55, height, segment + 0.12))
    _wall(root, prefix + "S", base + Vector3(0, height * 0.5, offset), Vector3(0.55, height, segment + 0.12))
    var door_height := 3.35
    var lintel_height: float = height - door_height
    _wall(root, prefix + "Lintel", base + Vector3(0, door_height + lintel_height * 0.5, 0), Vector3(0.55, lintel_height, opening_width + 0.16))

func _door_trim_x(root: Node3D, base: Vector3, opening_width: float, opening_height: float) -> void:
    _box(root, "DoorTrimL", base + Vector3(-opening_width * 0.5 - 0.12, opening_height * 0.5, 0), Vector3(0.20, opening_height, 0.18), STONE_PALE, false)
    _box(root, "DoorTrimR", base + Vector3(opening_width * 0.5 + 0.12, opening_height * 0.5, 0), Vector3(0.20, opening_height, 0.18), STONE_PALE, false)
    _box(root, "DoorTrimTop", base + Vector3(0, opening_height + 0.10, 0), Vector3(opening_width + 0.44, 0.20, 0.18), STONE_PALE, false)
...
func _partition_x_with_opening(root: Node3D, center: Vector3, total_length: float, opening: float) -> void:
    ...
    _wall(root, "DormPartitionL", center + Vector3(-offset, 1.55, 0), Vector3(segment + 0.10, 3.1, 0.30))
    _wall(root, "DormPartitionR", center + Vector3(offset, 1.55, 0), Vector3(segment + 0.10, 3.1, 0.30))

func _corner_trim(root: Node3D, pos: Vector3, height: float) -> void:
    _box(root, "StoneCorner", pos + Vector3(0, height * 0.5, 0), Vector3(0.72, height, 0.72), STONE_DARK)
...
func _shelf(root: Node3D, pos: Vector3, yaw: float) -> void:
    ...
    shelf_root.rotation_degrees.y = yaw
    root.add_child(shelf_root)
    _box(shelf_root, "ShelfBack", Vector3(0, 1.35, -0.30), Vector3(3.2, 2.7, 0.22), WOOD)
    for y in [0.30, 0.93, 1.56, 2.19]:
        _box(shelf_root, "ShelfBoard", Vector3(0, y, 0.02), Vector3(3.2, 0.12, 0.72), WOOD)
    for row in range(3):
        for col in range(7):
            var tone: float = 0.16 + float((row + col) % 4) * 0.035
            var book_color := Color(0.26 + tone, 0.18 + tone * 0.45, 0.10 + tone * 0.25)
            _box(shelf_root, "Volume", Vector3(-1.28 + col * 0.42, 0.57 + row * 0.63, 0.30), Vector3(0.28, 0.46, 0.24), book_color, false)
...
func _chimes_z(root: Node3D, pos: Vector3) -> void:
    _box(root, "ChimeBeam", pos, Vector3(0.12, 0.08, 1.0), WOOD, false)
    for i in range(4):
        ...
        tube.mesh = tube_mesh
        tube.position = pos + Vector3(0, -0.36 - float(i) * 0.04, -0.36 + float(i) * 0.24)
        root.add_child(tube)
