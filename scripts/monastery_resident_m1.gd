...
# Milestone 1 resident pass: naturalized dialogue plus corrected Sargasson
# anatomy. Physical hit reactions stay for Milestone 2.
...
func _keeper_dialogue() -> Dictionary:
    ...
    if current_page == "keeper_plants":
        return {
            "speaker": display_name,
            "text": "Most of the green things here came from somewhere kinder. Iustitia keeps them alive because generations of stubborn monks refuse to let the cuttings die.",
            "choices": [{"text": "Back.", "action": "back"}]
        }
    return {
        "speaker": display_name,
