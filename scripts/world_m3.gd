...
# Milestone 3 restores one compact, canonical-feeling garden precinct without
# importing the earlier oversized experimental garden geometry.
func _build_world() -> void:
    super._build_world()
    _open_west_cloister()
    _build_west_cloister()

func _rock(pos: Vector3, rock_scale: Vector3, color: Color) -> void:
    # The old southwest boulder occupied the footprint of the west cloister.
    # Move that single legacy scenery piece rather than layering a garden over it.
    if absf(pos.x + 25.0) < 0.05 and absf(pos.z - 16.0) < 0.05:
        super._rock(Vector3(-38.0, pos.y, 18.0), rock_scale, color)
        return
    super._rock(pos, rock_scale, color)

func _open_west_cloister() -> void:
    var old_wall := get_node_or_null("WestWall")
    if old_wall != null:
        old_wall.queue_free()

    # Five-meter opening centered on z=4.5. The pieces deliberately retain the
    # old courtyard wall's height and mass so this reads as a carved threshold,
    # not as the courtyard simply ending.
...
func _build_west_cloister() -> void:
    var root := Node3D.new()
    root.name = "WestBotanicalCloister"
    add_child(root)
    ...
    # Sparse peristyle: enough to read as monastery architecture, not a park.
    for z in [-2.2, 1.2, 4.6, 8.0, 11.4]:
        _column(Vector3(-31.0, 2.45, z), STONE_PALE.darkened(0.03))

    _build_m3_eytelia_tree(Vector3(-24.8, 0.0, 4.4))
    _build_m3_pampin_vines(Vector3(-29.2, 0.0, -2.7), 8.7)
    _build_m3_pampin_vines(Vector3(-29.2, 0.0, 11.8), 8.7)
    ...
    _alien_plant(Vector3(-18.8, 0.0, 6.8))
    _alien_plant(Vector3(-29.2, 0.0, 5.5))

func _build_m3_eytelia_tree(pos: Vector3) -> void:
...
func _build_m3_pampin_vines(pos: Vector3, length: float) -> void:
    # The support and strands are decorative so a hanging vine cannot create an
    # invisible collision snag across a walking route.
    _box("PampinBeam", pos + Vector3(length * 0.5, 2.9, 0), Vector3(length, 0.18, 0.20), Color(0.21, 0.15, 0.09), false)
    var root := Node3D.new()
    root.name = "PampinVines"
    root.position = pos
    add_child(root)
    for i in range(8):
...
func _m3_flora_fontis(pos: Vector3) -> void:
    var root := Node3D.new()
    root.name = "FloraFontis"
    root.position = pos
    add_child(root)
    for i in range(4):
        ...
        stalk.position = Vector3(-0.18 + float(i) * 0.12, 0.22 + float(i) * 0.02, 0.04 * float(i % 2))
